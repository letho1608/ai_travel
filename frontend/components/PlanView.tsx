...
const tripDestination=(plan:Plan)=>{
  if(typeof plan.diem_den==="string"&&plan.diem_den.trim()&&!/^(việt\s*nam|vietnam)$/i.test(plan.diem_den.trim()))return plan.diem_den.trim();
  const understood=(plan as Record<string,unknown>).dau_vao_da_hieu;
  if(isRecord(understood)&&isRecord(understood.diem_den)){
    const value=understood.diem_den.gia_tri;
    if(typeof value==="string"&&value.trim())return value.trim();
    if(isRecord(value)&&typeof value.ten==="string"&&value.ten.trim())return value.ten.trim();
  }
  return plan.ngay.flatMap(day=>day.khoang_gio).map(slot=>slot.khu_vuc?.trim()).find(Boolean)||"";
};
...
const HERO_SKIP_KINDS=new Set(["cafe","ca_phe","nha_hang","quan_an","cho","drinks","khach_san"]);
const HERO_SKIP_URL=/zojirushi|coffeemaker|coffee.?maker|consumer.?reports|may.?pha.?ca.?phe/i;
const isHeroPhoto=(slot:Slot,url:string)=>{
  const kind=String(slot.loai||"").toLowerCase();
  if(HERO_SKIP_KINDS.has(kind))return false;
  const name=slot.ten_dia_diem.toLowerCase();
  if(name.includes("cafe")||name.includes("cà phê")||name.includes("coffee"))return false;
  return !HERO_SKIP_URL.test(url);
};
...
export default function PlanView({initial,token,version,constraints:initialConstraints}:{initial:Plan;token:string;version:number;constraints?:ThamSo}){
  ...
  const coverUrl=(()=>{const cover=safeImageUrl(plan.anh_bia);if(cover&&!brokenImages.has(cover)&&!HERO_SKIP_URL.test(cover))return cover;for(const day of plan.ngay){for(const slot of day.khoang_gio){const url=safeImageUrl(slot.anh);if(url&&!brokenImages.has(url)&&isHeroPhoto(slot,url))return url}}return null})();
  const coverAlt=plan.tieu_de;
  ...
  return <main className="workspace-page">
    ...
    <div className="workspace"><aside className="chat-panel card" aria-label={t("tripAssistant")}><div className="panel-title"><span className="assistant-dot"/>{t("tripAssistant")}</div><div className="messages" aria-live="polite">{conversation.map((item,index)=><div key={index} className={`bubble ${item.role}`}>{item.key?t(item.key):item.text}</div>)}</div><form className="chat-box" onSubmit={sendChat}><input value={chat} disabled={disabled} onChange={event=>setChat(event.target.value)} placeholder={t("chatPlaceholder")} aria-label={t("chatPlaceholder")}/><button disabled={disabled} aria-label={t("send")}>↑</button></form></aside>
      ...
      <section className="map-panel"><MapView slots={slots} selectedId={selectedId} onSelect={setSelectedId} destination={tripDestination(plan)} title={plan.tieu_de}/><div className="map-legend card">{t("mapLegend")}</div></section></div>
