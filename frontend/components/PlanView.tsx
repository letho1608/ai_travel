...
export default function PlanView({initial,token,version,constraints:initialConstraints}:{initial:Plan;token:string;version:number;constraints?:ThamSo}){
  ...
  const disabled=busy!==null,quickActions=locale==="vi"?quickRefines.vi:quickRefines.en;
  ...
  return <main className="workspace-page">
    ...
    <header className="trip-header"><div><div className="eyebrow">{t("workspaceEyebrow")}</div><h1>{plan.tieu_de}</h1><p>{plan.tom_tat}</p></div><div className="trip-actions export-actions" aria-label={t("itinerary")}><a className="secondary button-link" href={`${API_URL}/api/plans/${token}/itinerary.pdf`}>{t("downloadPdf")}</a><a className="secondary button-link" href={`${API_URL}/api/plans/${token}/calendar.ics`}>{t("addCalendar")}</a></div></header>
