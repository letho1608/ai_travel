...
const extLinks=(name:string,lat:number,lng:number)=>{const q=encodeURIComponent(`${name} Hà Nội`);return{maps:`https://www.google.com/maps/search/?api=1&query=${lat},${lng}`,google:`https://www.google.com/search?q=${q}`,tiktok:`https://www.tiktok.com/search?q=${q}`}};
const kindEmoji=(kind:string|undefined)=>({cafe:"☕",nha_hang:"🍽️",quan_an:"🍜",cho:"🛍️",cong_vien:"🌳",dia_danh:"🏛️",bao_tang:"🖼️",di_tich:"🏯",chua:"🛕",den:"⛩️",khach_san:"🛏️",khac:"📍"})[kind||"khac"]||"📍";
...
export default function PlanView({initial,token,version,constraints:initialConstraints}:{initial:Plan;token:string;version:number;constraints?:ThamSo}){
  ...
  const slotPhoto=(slot:Slot)=>{const url=safeImageUrl(slot.anh);return url&&!brokenImages.has(url)?<div className="slot-photo"><Image src={url} alt="" fill sizes="(max-width:760px) 100vw, 33vw" loading="lazy" referrerPolicy="no-referrer" unoptimized onError={()=>hideImage(url)}/></div>:<div className={`slot-photo placeholder kind-${slot.loai||"other"}`} role="img" aria-hidden="true"><span>{kindEmoji(slot.loai)}</span></div>};

  return <main className="workspace-page">
    <button type="button" className="result-back-to-chat" onClick={returnToChat}><svg aria-hidden="true" viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/><path d="M9 12h10"/></svg>{t("backToChat")}</button>
    <section className="result-ready card" aria-labelledby="result-ready-title"><span className="result-ready-badge"><span aria-hidden="true">✦</span> {t("aiFinished")}</span><h2 id="result-ready-title">{t("itinerarySuggestionTitle")}</h2><p>{t("itinerarySuggestionLead")}</p></section>
    <header className="trip-header"><div><div className="eyebrow">{t("workspaceEyebrow")}</div><h1>{plan.tieu_de}</h1><p>{plan.tom_tat}</p></div><div className="trip-actions"><button className="secondary" onClick={copy} disabled={disabled}>{t("share")}</button><a className="secondary button-link" href={`${API_URL}/api/plans/${token}/itinerary.pdf`}>{t("downloadPdf")}</a><a className="secondary button-link" href={`${API_URL}/api/plans/${token}/calendar.ics`}>{t("addCalendar")}</a><button className="secondary" onClick={downloadJson} disabled={disabled}>{t("downloadJson")}</button><button className="secondary" onClick={()=>setShowComments(value=>!value)}>{t("comments")} {comments.length}</button>{canFeedback&&<button className="secondary" onClick={()=>setShowFeedback(value=>!value)}>{t("feedback")}</button>}<button className="secondary" onClick={loadVersions} disabled={disabled}>{t("versions",{version:ver})}</button>{ver>1&&<button className="secondary" onClick={()=>restore(ver-1)} disabled={disabled}>{t("undo")}</button>}<button className="secondary" onClick={regenerate} disabled={disabled}>{t("regenerate")}</button></div></header>
    <div className="trip-facts" aria-label={t("tripSummary")}><span>☁ {plan.thoi_tiet.tinh_trang}{plan.thoi_tiet.nhiet_do_max!==undefined?` · ${unit==="imperial"?Math.round(plan.thoi_tiet.nhiet_do_max*9/5+32):plan.thoi_tiet.nhiet_do_max}°${unit==="imperial"?"F":"C"}`:""}</span><span>{money.format(plan.chi_phi_moi_nguoi)} / {t("perPerson")}</span><span>{t("places",{count:slots.length})}</span>{constraints&&<span>{t("tripConstraints",{budget:money.format(constraints.ngan_sach),people:constraints.so_nguoi,duration:t(durationKeys[constraints.thoi_luong]??constraints.thoi_luong)})}</span>}{plan.ngay_cap_nhat&&!Number.isNaN(new Date(plan.ngay_cap_nhat).getTime())&&<span className="last-updated">{t("lastUpdated",{time:date.format(new Date(plan.ngay_cap_nhat))})}</span>}</div>
