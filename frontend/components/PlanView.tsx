...
export default function PlanView({initial,token,version,constraints:initialConstraints}:{initial:Plan;token:string;version:number;constraints?:ThamSo}){
  ...
  return <main className="workspace-page">
    <section className="result-ready card" aria-labelledby="result-ready-title"><div className="result-ready-copy"><span className="result-ready-badge"><span aria-hidden="true">✦</span> {t("aiFinished")}</span><h2 id="result-ready-title">{t("itinerarySuggestionTitle")}</h2><p>{t("itinerarySuggestionLead")}</p></div><button type="button" className="result-back-to-chat" onClick={returnToChat}><svg aria-hidden="true" viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/><path d="M9 12h10"/></svg>{t("backToChat")}</button></section>
    <header className="trip-header card"><div><div className="eyebrow">{t("workspaceEyebrow")}</div><h1>{plan.tieu_de}</h1><p>{plan.tom_tat}</p></div><div className="trip-facts" aria-label={t("tripSummary")}><span>☁ {plan.thoi_tiet.tinh_trang}{plan.thoi_tiet.nhiet_do_max!==undefined?` · ${unit==="imperial"?Math.round(plan.thoi_tiet.nhiet_do_max*9/5+32):plan.thoi_tiet.nhiet_do_max}°${unit==="imperial"?"F":"C"}`:""}</span><span>{money.format(plan.chi_phi_moi_nguoi)} / {t("perPerson")}</span><span>{t("places",{count:slots.length})}</span>{constraints&&<span>{t("tripConstraints",{budget:money.format(constraints.ngan_sach),people:constraints.so_nguoi,duration:t(durationKeys[constraints.thoi_luong]??constraints.thoi_luong)})}</span>}{plan.ngay_cap_nhat&&!Number.isNaN(new Date(plan.ngay_cap_nhat).getTime())&&<span className="last-updated">{t("lastUpdated",{time:date.format(new Date(plan.ngay_cap_nhat))})}</span>}</div></header>
