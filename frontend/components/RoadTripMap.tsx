...
export default function RoadTripMap({geometry,stops,label}:{geometry:{coordinates:number[][]};stops:Stop[];label:string}) {
  ...
  useEffect(()=>{
    ...
    L.polyline(points,{color:"#086b27",weight:5}).addTo(map);
    stops.forEach((stop,index)=>L.circleMarker([stop.location.lat,stop.location.lng],{radius:9,color:index===0?"#bb4d45":"#086b27",fillOpacity:1}).bindTooltip(`${index+1}. ${stop.name}`).addTo(map));
