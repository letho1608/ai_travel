...
export default function RoadTripMap({geometry,stops,label}:{geometry:{coordinates:number[][]};stops:Stop[];label:string}) {
  ...
  useEffect(()=>{
    ...
    L.polyline(points,{color:"#0f766e",weight:5}).addTo(map);
    stops.forEach((stop,index)=>L.circleMarker([stop.location.lat,stop.location.lng],{radius:9,color:index===0?"#e4572e":"#0f766e",fillOpacity:1}).bindTooltip(`${index+1}. ${stop.name}`).addTo(map));
