export type Slot = { bat_dau: string; ket_thuc: string; dia_diem_id: string; ten_dia_diem: string; loai: string; mo_ta: string; chi_phi: number; toa_do: {lat:number; lng:number}; ghi_chu:string; nguon?:string; nguon_url?:string|null; anh?:string|null; anh_nguon?:string|null };
export type HoiThoaiTurn = { vai_tro:"user"|"assistant"; noi_dung:string; thoi_gian:string };
export type Plan = { tieu_de:string; tom_tat:string; thoi_luong:string; ngay_di?:string; tong_chi_phi:number; chi_phi_moi_nguoi:number; thoi_tiet:{tinh_trang:string;ghi_chu:string;nhiet_do_min?:number;nhiet_do_max?:number;xac_suat_mua?:number;nguon?:string}; ngay:Array<{thu_tu:number;nhan_de:string;khoang_gio:Slot[]}>; luu_y:string[]; hoi_thoai?:HoiThoaiTurn[]; ngay_cap_nhat?:string };
