...
import { SERVER_API_URL, BASE_URL } from "@/lib/api";
async function load(token:string){const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),8000);try{const r=await fetch(`${SERVER_API_URL}/api/plans/${token}`,{cache:"no-store",signal:controller.signal});if(!r.ok)throw new Error("Kế hoạch không tồn tại hoặc đã hết hạn");return r.json()}finally{clearTimeout(timeout)}}
