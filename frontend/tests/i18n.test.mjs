...
test("workspace mutations fail safely and guard duplicate actions",()=>{
  ...
  assert.match(mapViewSource,/function placeReviewQuery\(slot: Slot, fallbackCity = ""\)/);
  assert.match(planViewSource,/const tripDestination=\(plan:Plan\)/);
  assert.match(mapViewSource,/\$\{name\} \$\{city\} review/);
  assert.doesNotMatch(mapViewSource,/Hanoi review/);
  assert.doesNotMatch(mapViewSource,/\$\{name\} Việt Nam/);
  assert.match(mapViewSource,/google\.com\/search\?q=\$\{query\}/);
  assert.match(mapViewSource,/tiktok\.com\/search\?q=\$\{query\}/);
  assert.match(mapViewSource,/function mapsHref\(/);
  assert.match(mapViewSource,/q=place_id:\$\{encodeURIComponent\(placeId\)\}/);
  assert.match(mapViewSource,/\/api\/maps\/place\?\$\{params\.toString\(\)\}/);
  assert.doesNotMatch(mapViewSource,/!8m2!3d\$\{lat\}!4d\$\{lng\}/);
  assert.match(googlePlacesSource,/GOOGLE_TEXT_SEARCH_URL/);
  assert.match(googlePlacesSource,/GOOGLE_NEARBY_SEARCH_URL/);
  assert.match(googlePlacesSource,/locationBias/);
  assert.doesNotMatch(googlePlacesSource,/query_text = f"\{name\} Việt Nam"/);
